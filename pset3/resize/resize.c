...
int main(int argc, char *argv[])
{
    ...
    // store height and width of input file
    LONG biWidthIn = bi.biWidth;
    LONG biHeightIn = bi.biHeight;
    ...
    for (int i = 0, biHeight = abs(biHeightIn); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            // determine line size of input file
            int lineSize = (sizeof(RGBTRIPLE) * biWidthIn) + paddingIn;
            
            // seek back to beginning of input file line
            if (j != (n - 1))
            {
